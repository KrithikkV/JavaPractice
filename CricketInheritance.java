...
class Batsman extends Cricketer {
 Batsman(String name) { super(name, "Batsman"); }
}
...
//Hierarchical Inheritance (Cricketer -> Bowler)
class Bowler extends Cricketer {
 Bowler(String name) { super(name, "Bowler"); }
}

//Multiple Inheritance (via Interface)
interface AllRounderSkills {
 void skillSet();
}

class AllRounder extends Cricketer implements AllRounderSkills {
 AllRounder(String name) { super(name, "All-Rounder"); }
     ...
     do {
         ...
         if (choice >= 1 && choice <= 4) {
             if (playerCount < team.length) {
                 ...
                 if (choice == 1) team[playerCount] = new Opener(pName);
                 else if (choice == 2) team[playerCount] = new Batsman(pName);
                 else if (choice == 3) team[playerCount] = new Bowler(pName);
                 else if (choice == 4) team[playerCount] = new AllRounder(pName);
